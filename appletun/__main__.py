...
def get_vpn_config_dir():
    """ Locate VPN config dir """
    if "darwin" == platform:
        return path.join(local['brew']('--prefix').strip(), 'etc')
    else:
        return "/etc"

def check_vpn_config_exist(config_dir:  str,
                           vpn_name:    str):
    """ Check if the vpn config already exists in ipsec.conf """
    ipsec_conf_path = path.join(config_dir, 'ipsec.conf')
    if path.exists(ipsec_conf_path):
        with open(ipsec_conf_path, 'r') as f:
            ipsec_conf = f.read()
            return f"conn {vpn_name}\n" in ipsec_conf

    return False

def write_new_vpn_config(config_dir:    str,
                         vpn_name:      str):
    """ Create a new config for a VPN and write it to ipsec.config if no config with the same name exists """
    ipsec_conf_path = path.join(config_dir, 'ipsec.conf')
    if not check_vpn_config_exist(config_dir, vpn_name):
        config = BASE_VPN_CONFIG_FORMAT.format(vpn_name=vpn_name)
        with open(ipsec_conf_path, 'a') as f:
            f.write(config)

def get_vpn_secret(config_dir:  str,
                   server_id:   str):
    """ Retrieve any existing secrets for a given server_id from ipsec.secrets"""
    secrets_path = path.join(config_dir, 'ipsec.secrets')
    if path.exists(secrets_path):
        with open(secrets_path, 'r') as f:
            return re.findall(f'{server_id}.*?:.*?PSK.*?"(.*)"', f.read())

    return []

def write_vpn_secret(config_dir:    str,
                     server_id:     str,
                     psk:           str):
    """ Write a new PSK secret to ipsec.secrets if not already exists"""
    existing = get_vpn_secret(config_dir, server_id)
...
def generate_profile(server_address:    str,
    ...
    profile = {'PayloadDisplayName': f'{vpn_name}VPN PSK {psk} HTTP {http_proxy_addr}:{http_proxy_port}',
    ...
    if http_proxy_cert is not None:
        profile['PayloadContent'].append({'PayloadIdentifier': 'com.apple.security.root.86FD9307-9CAC-4401-ABD4-FE0F70521A62',
...
def generate_and_install_profile(service: MobileConfigService,
    ...
    config_dir = get_vpn_config_dir()
    if psk is None:
        vpn_secrets = get_vpn_secret(config_dir, vpn_name)
    ...
    if write_config:
        write_new_vpn_config(config_dir, vpn_name)
        write_vpn_secret(config_dir, vpn_name, psk)
...
def start(service_provider:   LockdownServiceProvider):
    """ Start AppleTun VPN """
...
@click.option('-p', '--psk', show_default='If one already exist for the same vpn name, use it, otherwise generated random PSK', help='PSK for the authentication with the VPN')
@click.option('-H', '--http_proxy-addr', show_default="Same as server address", help='Address of http proxy')
@click.option('-P', '--http-proxy-port', type=click.IntRange(0x0000,0xffff), default=9090, show_default=True, help='Port for http proxy')
@click.option('-C', '--http_proxy-cert', type=click.File('rb'), default=None, show_default="No certificate would be installed", help='Certificate for the http proxy to install with the profile')
@click.option('--write-config/--no-write-config', default=True, help='Write the VPN configuration to ipsec.config/ipsec.secrets')
def install_profile(service_provider:   LockdownServiceProvider,
    ...
    generate_and_install_profile(service, server_address, vpn_name, psk, http_proxy_port, http_proxy_addr, raw_cert, write_config)
...
@cli.command(cls=Command)
def remove_profile(service_provider:   LockdownServiceProvider):
